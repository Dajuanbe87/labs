...
int main() {
    ...
    while (true) {
        ...
        case 1: {
            ...
            for (int a = 0; a < numCity; a++) {
                for (int b = 0; b < numDay; b++) {
                    do {
                        cout << "Add the value of day #" << b + 1 << " for city #" << a + 1 << " (°C): ";
                        cin >> z;
                        if (z <= -61 || z >= 101) {
            ...
            for (int c = 0; c < temperatures.size(); c++) {
                ...
                for (int d = 0; d < temperatures[c].size(); d++) {
                    cout << "Day #" << d + 1 << ": " << temperatures[c][d] << " °C" << endl;
                }
            }
            cout << "\n";
            break;
        ...
        case 2:{
            ...
            int desiscion;
            cout << "\nyou want to do" << "\n1.eliminate a specific day" << "\n2.elminate a city" << endl;
            ...
            if (desiscion == 1) {
                ...
                cout << "Which day do you want to delete?\nYour select: ";
                int tempDay;
                cin >> tempDay;

                // Eliminar el día específico de la ciudad seleccionada
                if (tempCity > 0 && tempCity <= temperatures.size() && tempDay > 0 && tempDay <= temperatures[tempCity - 1].size()) {
                    temperatures[tempCity - 1].erase(temperatures[tempCity - 1].begin() + tempDay - 1);
                }
                else {
                    cout << "Invalid city or day selection." << endl;
                }
                ...
                cout << "what city eliminet?" << endl;
                int elimtcity;
                cin >> elimtcity;

                // Eliminar la ciudad seleccionada
                if (elimtcity > 0 && elimtcity <= temperatures.size()) {
                    temperatures.erase(temperatures.begin() + elimtcity - 1);
                    temperatures.erase(temperatures.begin() + elimtcity - 1);
        ...
        case 3: {
            ...
            if (tempCity > 0 && tempCity <= temperatures.size()) {
                ...
                for (int e = 0; e < temperatures[tempCity - 1].size(); e++) {
                    cout << "Day #" << e + 1 << ": " << temperatures[tempCity - 1][e] << " °C" << endl;
                }
                ...
                if (tempDay > 0 && tempDay <= temperatures[tempCity - 1].size()) {
                    float tempSelection;
                    do {
                        cout << "\nData of day selected: " << temperatures[tempCity - 1][tempDay - 1] << " °C";
                        cout << "\nEnter the value for which you want to change (°C): ";
                        cin >> tempSelection;
            ...
            for (int f = 0; f < temperatures.size(); f++) {
                ...
                for (int g = 0; g < temperatures[f].size(); g++) {
                    cout << "Day #" << g + 1 << ": " << temperatures[f][g] << " °C" << endl;
                }
            }
            break;
        }
        ...
        case 5: {
            ...
            for (size_t day = 0; day < numDays; ++day) {
                ...
                cout << "Day #" << (day + 1) << endl;
                cout << "Average Temperature: " << avgTemp << " °C" << endl;
                cout << "Maximum Temperature: " << dayMax[day] << " °C" << endl;
                cout << "Minimum Temperature: " << dayMin[day] << " °C" << endl;
                cout << "\n";
            }
            break;
        }
